...
function FullCastCrewPage({movieDetails, castAndCrew}){

    const { title, id } = movieDetails;
    const casts = castAndCrew.cast;
    const crews = castAndCrew.crew;
                ...
                <div className="w-1/2">
                    <h1 className="md:p-0 text-2xl md:text-3xl font-weight-bolder mb-1 text-orange-600 whitespace-normal">Cast</h1>
                        {casts.length !== 0 ? casts.map((cast, i) => 
                            <div key={i}>
                               <img className="w-12 md:w-16 rounded" src={cast.profile_path ? 'https://image.tmdb.org/t/p/w500' + cast.profile_path : process.env.PUBLIC_URL + '/img/alt-2.jpg'} alt="No Profile pic"/>
                                 <a href={'/person/' + cast.id}>
                                    <h2 className="text-sm md:text-base px-6 py-2 hover:text-gray-500">{cast.name}{' | '}{cast.character}</h2>
                                </a>  
                            </div>
                        ) : <h2 className="text-sm md:text-base px-6 py-2 hover:text-gray-500">No found result</h2>}
                </div>
                <div className="w-1/2">
                    <h1 className="md:p-0 text-2xl md:text-3xl font-weight-bolder mb-1 text-orange-600 whitespace-normal">Crew</h1>
                        {crews.length !== 0 ? crews.map((crew, i) => 
                            <div key={i}>
                                <img className="w-12 md:w-16 rounded" src={crew.profile_path ? 'https://image.tmdb.org/t/p/w500' + crew.profile_path : process.env.PUBLIC_URL + '/img/alt-2.jpg'} alt="No Profile pic"/>
